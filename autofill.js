  ...
  if (input.value !== '') {
    ...
    for (let i of sortedIngredientsList) {
      if (
        ...
        listItem.classList.add("autofill-items");
        listItem.style.cursor = "pointer";
        listItem.setAttribute("onclick", "displayAutofill('" + i + "')");

        let word =
          "<b>" + i.substr(0, input.value.length).toLowerCase() + "</b>";
        word += i.substr(input.value.length);

        listItem.innerHTML = word;
        autofillDropdown.appendChild(listItem);
      }
    }
  }
});

function displayAutofill(value) {
  input.value = value.toLowerCase();
  clearDropdown();
}

function clearDropdown() {
  let items = document.querySelectorAll(".autofill-items");
